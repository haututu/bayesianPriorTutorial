...
We then construct simple weak priors for the logistic regression, and we do this out of neccesity because the dataset is so large that it would take a day to run. When using Stan as your MCMC sampler, even very weak priors can have a major impact on your convergence time without influencing your posterior to any appreciable degree.
...
```{r}
syntheticNz <- readRDS("data/syntheticNz20191204.RDS")
syntheticNz <- within(syntheticNz, sexident <- relevel(sexident, ref = "Heterosexual"))
syntheticNz <- within(syntheticNz, sex <- relevel(sex, ref = "M"))
syntheticNz <- mutate(syntheticNz, rand = runif(1000), k10high = ifelse(hazardous == 1 & k10high == 0 & rand < 0.02, 1, 0))
```
